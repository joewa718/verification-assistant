...
import com.nielsen.verfication.measure.configuration.enums.{ArrayFlattenType, BatchProcessType, DefaultFlattenType, FlattenType, MetricOutputType, RecordOutputType, StreamingProcessType}
...
import com.nielsen.verfication.measure.step.builder.dsl.expr.{Expr, TimelinessClause}
import com.nielsen.verfication.measure.step.builder.dsl.transform.analyzer.TimelinessAnalyzer
import com.nielsen.verfication.measure.step.transform.SparkSqlTransformStep
import com.nielsen.verfication.measure.step.write.{MetricWriteStep, RecordWriteStep}
import com.nielsen.verfication.measure.utils.TimeUtil
...
import com.nielsen.verfication.measure.step.write.MetricWriteStep
    ...
    if (!context.runTimeTableRegister.existsTable(sourceName)) {
      ...
      val (transSteps3, writeSteps3) = TimeUtil.milliseconds(details.getString(_stepSize, "")) match {
        case Some(stepSize) =>
          ...
          val rangeMetricWriteStep = {
            write.MetricWriteStep(stepColName, rangeMetricTableName, ArrayFlattenType)
          }
      ...
      val (transSteps4, writeSteps4) = if (percentiles.size > 0) {
        ...
        val percentileWriteStep = {
          write.MetricWriteStep(percentileTableName, percentileTableName, DefaultFlattenType)
        }
