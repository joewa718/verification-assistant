...
under the License.
*/
package com.nielsen.verfication.measure.launch.streaming

import java.util.{Date, Timer, TimerTask}
import java.util.concurrent.{Executors, ThreadPoolExecutor, TimeUnit}

import com.nielsen.verfication.measure.Loggable
import com.nielsen.verfication.measure.configuration.dqdefinition.{DQConfig, EnvConfig, EvaluateRuleParam, GriffinConfig}
import com.nielsen.verfication.measure.configuration.enums.StreamingProcessType
import com.nielsen.verfication.measure.context.{ContextId, DQContext}
import com.nielsen.verfication.measure.context.streaming.checkpoint.offset.OffsetCheckpointClient
import com.nielsen.verfication.measure.context.streaming.metric.CacheResults
...
import com.nielsen.verfication.measure.launch.DQApp
import com.nielsen.verfication.measure.step.builder.udf.GriffinUDFAgent
import com.nielsen.verfication.measure.utils.{HdfsUtil, TimeUtil}
...
case class StreamingDQApp(allParam: GriffinConfig, messageSeq: ArrayBuffer[String]) extends DQApp {
  ...
  case class StreamingDQCalculator(globalContext: DQContext,
    ...
    def run(): Unit = {
      ...
      info(s"===== [${updateTimeDate}] process begins =====")
      ...
      if (locked) {
        ...
        info(s"===== [${updateTimeDate}] process ignores =====")
      }
      val endTime = new Date().getTime
      info(s"===== [${updateTimeDate}] process ends, using ${endTime - updateTime} ms =====")
    }
