...
under the License.
*/
package com.nielsen.verfication.measure.configuration.dqdefinition

import com.fasterxml.jackson.annotation.{JsonInclude, JsonProperty}
import com.fasterxml.jackson.annotation.JsonInclude.Include
import com.nielsen.verfication.measure.configuration.enums.{DqType, DslType, FlattenType, GriffinDslType, OutputType, SinkType, UnknownType}
...
case class RuleParam(@JsonProperty("dsl.type") private val dslType: String,
                     ...
                     @JsonProperty("assert") private val asserts: List[RuleAssertParam] = null
                    ) extends Param {
  ...
  def getAsserts: Seq[RuleAssertParam] = if (asserts != null) asserts else Nil
  ...
  def replaceInDfName(newName: String): RuleParam = {
    ...
    else RuleParam(dslType, dqType, newName, outDfName, rule, details, cache, outputs, asserts)
  }

  def replaceOutDfName(newName: String): RuleParam = {
    if (StringUtils.equals(newName, outDfName)) this
    else RuleParam(dslType, dqType, inDfName, newName, rule, details, cache, outputs, asserts)
  }
  ...
  def replaceInOutDfName(in: String, out: String): RuleParam = {
    ...
    else RuleParam(dslType, dqType, in, out, rule, details, cache, outputs, asserts)
  }

  def replaceRule(newRule: String): RuleParam = {
    if (StringUtils.equals(newRule, rule)) this
    else RuleParam(dslType, dqType, inDfName, outDfName, newRule, details, cache, outputs, asserts)
  }
...
/**
 * assert param of verification
 *
 * @param express assert type (must)
 * @param name    output name (optional)
 */
@JsonInclude(Include.NON_NULL)
case class RuleAssertParam(@JsonProperty("express") private val express: Seq[String],
                           @JsonProperty("name") private val name: String
                          ) extends Param {
  def getAssertExpress(): Option[Seq[String]] = if (express != null) Some(express) else None
