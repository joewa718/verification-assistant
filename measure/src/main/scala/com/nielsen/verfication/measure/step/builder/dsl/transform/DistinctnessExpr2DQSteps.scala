...
import com.nielsen.verfication.measure.configuration.enums.{ArrayFlattenType, EntriesFlattenType, RecordOutputType, StreamingProcessType}
...
import com.nielsen.verfication.measure.step.builder.dsl.expr.{DistinctnessClause, Expr}
import com.nielsen.verfication.measure.step.builder.dsl.transform.analyzer.DistinctnessAnalyzer
  ...
  def getDQSteps(): Seq[DQStep] = {
    ...
    if (!context.runTimeTableRegister.existsTable(sourceName)) {
      ...
      val distMetricWriteStep = {
        write.MetricWriteStep(distColName, distTableName, EntriesFlattenType, writeTimestampOpt)
      }
      ...
      val (transSteps4, writeSteps4) = if (duplicationArrayName.nonEmpty) {
        ...
        if (groupAliases.size > 0) {
          ...
          val groupDupMetricWriteStep = {
            write.MetricWriteStep(duplicationArrayName,
