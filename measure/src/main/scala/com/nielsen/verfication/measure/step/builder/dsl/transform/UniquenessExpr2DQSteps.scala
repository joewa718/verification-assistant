...
import com.nielsen.verfication.measure.configuration.enums.{ArrayFlattenType, BatchProcessType, EntriesFlattenType, RecordOutputType, StreamingProcessType}
...
import com.nielsen.verfication.measure.step.builder.dsl.expr.{Expr, UniquenessClause}
import com.nielsen.verfication.measure.step.builder.dsl.transform.analyzer.UniquenessAnalyzer
...
case class UniquenessExpr2DQSteps(context: DQContext,
  ...
  def getDQSteps(): Seq[DQStep] = {
    ...
    if (!context.runTimeTableRegister.existsTable(sourceName)) {
      ...
      info(s"[${timestamp}] data source ${targetName} not exists")
      Nil
    } else {
      ...
      val uniqueMetricWriteStep =
        write.MetricWriteStep(uniqueColName, uniqueTableName, EntriesFlattenType)
      ...
      val (transSteps2, writeSteps2) = if (duplicationArrayName.nonEmpty) {
        ...
        val dupMetricWriteStep = {
          write.MetricWriteStep(duplicationArrayName, dupMetricTableName, ArrayFlattenType)
        }
