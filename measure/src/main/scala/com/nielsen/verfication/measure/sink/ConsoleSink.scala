...
under the License.
*/
package com.nielsen.verfication.measure.sink
...
/**
  ...
  */
case class ConsoleSink(config: Map[String, Any], metricName: String, timeStamp: Long) extends Sink with Loggable{
  ...
  def start(msg: String): Unit = {
    info(s"[${timeStamp}] ${metricName} start: ${msg}")
  }
  def finish(): Unit = {
    info(s"[${timeStamp}] ${metricName} finish")
  }

  def log(rt: Long, msg: String): Unit = {
    info(s"[${timeStamp}] ${rt}: ${msg}")
  }
...
//    }
  ...
  def sinkMetrics(metrics: Map[String, Any]): Unit = {
    info(s"${metricName} [${timeStamp}] metrics: ")
