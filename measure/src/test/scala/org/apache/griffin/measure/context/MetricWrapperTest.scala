...
import com.nielsen.verfication.measure.context.MetricWrapper
...
class MetricWrapperTest extends FlatSpec with Matchers {
  ...
  it should "test" in {
    class SimpleParser extends RegexParsers {
      def word: Parser[String] = """[a-z]+""".r ~ """[a-z]+""".r ^^ {
        _._1.toString
      }

      def print(msg: String) = {
        val result = parse(word,msg)
        println(result)
      }
    }
    val simpleParser= new SimpleParser()
    simpleParser.print("johnny come lately")
  }
